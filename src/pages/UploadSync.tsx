...
import {
  Upload, Download, FileSpreadsheet, CheckCircle2, XCircle, AlertTriangle, ArrowRight, Shield, Database,
} from 'lucide-react';
import {
...
import { loadDB, saveDB } from '@/lib/mini-supabase';
import { playSyncSuccess, playError } from '@/lib/sound-engine';
...
export default function UploadSync() {
  ...
  // Fee collector can access upload sync
  const canAccess = isAdmin || role === 'handler' || role === 'fee_collector';
  ...
  const handleExportJSON = async () => {
    try {
      ...
      a.href = url;
      a.download = `erp-database-${currentFY}-${new Date().toISOString().slice(0, 10)}.json`;
      a.click();
      URL.revokeObjectURL(url);
      playSyncSuccess();
      toast({ title: 'Database exported', description: 'JSON database file downloaded.' });
    } catch (err: any) {
      playError();
  ...
  const handleImportJSON = async (file: File) => {
    ...
    try {
      ...
      // Validate structure
      const requiredKeys = ['clients', 'payments', 'invoices', 'handlers'];
      const missing = requiredKeys.filter(k => !Array.isArray(data[k]));
      if (missing.length > 0) {
        throw new Error(`Invalid JSON: missing arrays for: ${missing.join(', ')}`);
      }
      ...
      const mergedDB = {
        ...existingDB,
        clients: data.clients || existingDB.clients,
        payments: data.payments || existingDB.payments,
        invoices: data.invoices || existingDB.invoices,
        invoice_items: data.invoice_items || existingDB.invoice_items || [],
        handlers: data.handlers || existingDB.handlers,
        users: existingDB.users, // Never overwrite users
        audit_logs: existingDB.audit_logs || [],
      };

      await saveDB(mergedDB as any);
      await refreshData();
      playSyncSuccess();
      toast({
        title: 'JSON imported successfully',
        description: `${data.clients?.length || 0} clients, ${data.payments?.length || 0} payments, ${data.invoices?.length || 0} invoices restored.`,
      });
    } catch (err: any) {
      playError();
    ...
    <div className="space-y-5">
      <div>
        <h1 className="erp-page-title flex items-center gap-2">
          <Upload className="w-5 h-5" /> Upload & Sync
        </h1>
        <p className="text-xs text-muted-foreground mt-0.5">
          Excel upload, JSON database import/export, and data sync.
        </p>
      </div>

      {/* JSON Database Sync Section */}
      <div className="erp-kpi-card space-y-3">
        <h3 className="text-sm font-semibold flex items-center gap-2">
          <Database className="w-4 h-4" /> JSON Database Sync
        </h3>
        <p className="text-xs text-muted-foreground">Import or export the complete ERP database as a JSON file for backup/restore.</p>
        <div className="flex gap-2">
          <Button size="sm" variant="outline" onClick={handleExportJSON} className="gap-1">
            <Download className="w-3.5 h-3.5" /> Export JSON Database
          </Button>
          <Button
            size="sm"
            variant="outline"
            ...
            className="gap-1"
          >
            <Upload className="w-3.5 h-3.5" /> {jsonImporting ? 'Importing...' : 'Import JSON Database'}
          </Button>
          <input
            ref={jsonInputRef}
            type="file"
            ...
            {parsedPayload.validation.errors.length > 0 && (
              <div className="bg-destructive/10 rounded p-2 space-y-1">
                <p className="text-xs font-semibold text-destructive flex items-center gap-1">
                  <XCircle className="w-3 h-3" /> {parsedPayload.validation.errors.length} Errors
                </p>
        ...
        {!importResult ? (
          <div className="space-y-2">
            <p className="text-xs text-muted-foreground">
              ...
              totaling <strong>{formatCurrency(parsedPayload?.validation.total_payment_amount || 0)}</strong>.
            </p>
            <div className="flex gap-2">
              <Button
                size="sm"
          ...
          <div className="space-y-2">
            <div className={`rounded p-3 text-xs ${importResult.success ? 'bg-green-500/10' : 'bg-destructive/10'}`}>
              ...
              <div className="mt-2 grid grid-cols-4 gap-2">
                <div><p className="text-muted-foreground">Processed</p><p className="font-bold erp-mono">{importResult.processed}</p></div>
                <div><p className="text-muted-foreground">Skipped</p><p className="font-bold erp-mono">{importResult.skipped}</p></div>
                <div><p className="text-muted-foreground">Amount</p><p className="font-bold erp-mono">{formatCurrency(importResult.totalAmountProcessed)}</p></div>
                <div><p className="text-muted-foreground">Timestamp</p><p className="font-bold erp-mono text-[10px]">{new Date(importResult.timestamp).toLocaleString('en-IN')}</p></div>
              </div>
            </div>
            <Button size="sm" variant="outline" onClick={handleReset} className="gap-1">Upload Another</Button>
          </div>
        )}
      </div>
    </div>

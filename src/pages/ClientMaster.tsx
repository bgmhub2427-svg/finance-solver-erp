    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        {!isViewer && (
          <Dialog open={open} onOpenChange={(next) => {
            if (next) setForm(f => ({ ...f, clientId: generateClientId() }));
            setOpen(next);
          }}>
      ...
      <div className="erp-kpi-card p-0 overflow-auto">
        <table className="erp-table">
          ...
          <tbody>
            {filtered.length === 0 && (
              <tr><td colSpan={isViewer ? 13 : 14} className="text-center py-8 text-muted-foreground text-sm">No clients found.</td></tr>
            )}
            {filtered.map(c => (
              <tr key={c.id}>
                ...
                <td className="erp-mono text-xs">{c.pan || '—'}</td>
                <td><span className={`erp-badge ${c.status === 'inactive' ? 'erp-badge-warning' : 'erp-badge-success'}`}>{(c.status || 'active').toUpperCase()}</span></td>
                ...
                <td className="erp-mono text-xs text-right">{formatCurrency(c.totalPending)}</td>
                <td>
                  {c.totalPending === 0 ? (
